...
public class CustomReport extends Logfile{
	 ...
	 */
	 ...
	 public String AssertionresultOutput(WebDriver driver, String sTestcaseName) {
			...
			new File(sScreenshots).mkdirs();
			String sScreenshotName = null;
			System.setProperty("org.uncommons.reportng.escape-output", "false");
			String image = null;
			try {
				DateTime dateTime = new DateTime();
				sScreenshotName = sTestcaseName + dateTime.yyyyMMDDHHmmssTime() + ".png";
				log.info("sTestcaseName:::::" + sScreenshotName);
				...
				FileOutputStream out = new FileOutputStream(sFilepath + sScreenshotName);
				out.write(imageData);
				out.close();
				log.info("The screenshot stored in location" + sFilepath + sScreenshotName);

			} catch (Exception e1) {
				log.error("Snapshot Error : " + e1.getMessage());
				// log.error("Exception caught in screenshotcreation block");
				e1.printStackTrace();
			}
			return "<a href=\"file:\\\\\\" + sFilepath + sScreenshotName
					+ "\" target=_blank><p align=\"left\"><h5>Error screenshot at "
